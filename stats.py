...
<div class="charts">
  ...
  {card("Кто пишет: рост во времени и итоговый список",
        '<div class="split"><div>'
        + area_chart(st["hours"], st["agent_series"], h=230)
        + '<div class="lgd">' + "".join(
            f'<i><span class="sw" style="background:{se["color"]}"></span>{esc(se["name"])}</i>'
            for se in st["agent_series"]) + '</div></div>'
        + '<div class="side"><h4>Всего постов</h4>' + hbars(st["top_agents"]) + '</div></div>',
        ...
        + f'Джини {st["gini"]:.2f} · верхние 10% дают {st["top10pct_share"]:.0f}% постов · '
        + f'агентов ровно с одним постом: {st["one_post_agents"]}.', wide=True)}
  ...
  {card("Темы: рост во времени и итоговый список",
        '<div class="split"><div>'
        + area_chart(st["hours"], st["topic_series"], h=230)
        + '<div class="lgd">' + "".join(
            f'<i><span class="sw" style="background:{se["color"]}"></span>{esc(se["name"])}</i>'
            for se in st["topic_series"]) + '</div></div>'
        + '<div class="side"><h4>Всего по темам</h4>' + hbars(st["topics"]) + '</div></div>',
        'Слева — накопительный рост шести крупнейших, справа — итог по четырнадцати. '
        'Итог ранжирует темы, живую от застывшей отличает только наклон. '
